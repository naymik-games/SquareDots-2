class preview extends Phaser.Scene {
  ...
  create() {
    ...
    var playText = this.add.bitmapText(625, 1153, 'topaz', 'PLAY', 80).setOrigin(.5).setTint(0xffffff).setAlpha(1).setInteractive();
  ...
  setupGoals() {
    ...
    Object.entries(levels[onLevel].win).forEach(([key, value]) => {
      ...
      if (key == 'bomb') {
        ...
        this.bombIcon = this.add.image(220 + x * 200, y, 'goal_icons', 8).setScale(.7).setAlpha(1);
        this.bombText = this.add.bitmapText(280 + x * 200, y, 'topaz', '0', 70).setOrigin(0, .5).setTint(0xffffff).setAlpha(1);
        this.bombGoal = value;
        this.bombText.setText(value);
        this.previewBox.add(this.bombIcon);
        this.previewBox.add(this.bombText);
